...
int feature_extraction_pipeline(int argc, char** argv) {
	...
	*/
	...
	boost::split(file_names, save_feature_dataset_names,
		boost::is_any_of(","));
	...
	for (int i = 0; i < num_features; ++i){
		LOG(INFO) << "Opening the feature files: " << file_names[i];
		std::ofstream* db = new std::ofstream(file_names[i], std::ios_base::binary|std::ios_base::out);
