...
#include "caffe/layers/triplet_clip_hinge_loss_layer.hpp"
#include "caffe/util/math_functions.hpp"
#include "caffe/util/io.hpp"

// Added by Fuchen Long in 7/28/2016
...
namespace caffe {
	...
	void TripletClipHingeLossLayer<Dtype>::LayerSetUp(
		...
		diff_sub_or_si.Reshape(batch, bottom[0]->channels(), 1, 1); // F-F+
		diff_sub_or_di.Reshape(batch, bottom[0]->channels(), 1, 1); // F-F-
		diff_pow_or_si.Reshape(batch, bottom[0]->channels(), 1, 1); // Pow (F-F+)
		diff_pow_or_di.Reshape(batch, bottom[0]->channels(), 1, 1); // Pow (F-F-)
		ave_or.Reshape(batch, bottom[0]->channels(), 1, 1);// Ave(F)
		ave_si.Reshape(batch, bottom[0]->channels(), 1, 1);// Ave(F+)
		ave_di.Reshape(batch, bottom[0]->channels(), 1, 1);// Ave(F-)
		sub_or.Reshape(batch*(frame_num - 1), bottom[0]->channels(), 1, 1);// SUB(F)
		sub_si.Reshape(batch*(frame_num - 1), bottom[0]->channels(), 1, 1);// SUB(F+)
		sub_di.Reshape(batch*(frame_num - 1), bottom[0]->channels(), 1, 1);// SUB(F-)
		pow_sub_or.Reshape(batch*(frame_num - 1), bottom[0]->channels(), 1, 1);// SUB(F)
		pow_sub_si.Reshape(batch*(frame_num - 1), bottom[0]->channels(), 1, 1);// SUB(F+)
		pow_sub_di.Reshape(batch*(frame_num - 1), bottom[0]->channels(), 1, 1);// SUB(F-)
